...
"use strict";function debounce(fn,delay){var timeoutID=null;return function(){clearTimeout(timeoutID);var args=arguments,that=this;timeoutID=setTimeout(function(){fn.apply(that,args)},delay)}}Vue.directive("debounce",function(el,binding){binding.value===binding.oldValue||isIE11||(el.oninput=debounce(function(ev){el.dispatchEvent(new Event("change"))},parseInt(binding.value)||300))});var store=new Vuex.Store({state:{artists:[]},mutations:{artists:function(state,_artists){state.artists=_artists}},actions:{fetchApi:function(context,obj){var expires=new Date((new Date).getTime()+36e4);fetch(obj.api).then(function(response){return response.json().then(function(json){context.commit("artists",json);var sessionObject={expiresAt:expires,list:json};sessionStorage.setItem(obj.types,JSON.stringify(sessionObject))})})},session:function session(context,obj){var session=JSON.parse(sessionStorage.getItem(obj.types));context.commit("artists",session.list)},touringApi:function(context,obj){context.commit("artists",obj)}},getters:{}}),tourApi="/v1/touingcaa/searchArtist/",isIE11=!!window.MSInputMethodContext&&!!document.documentMode,globalSearch=new Vue({el:"#apps",store:store,data:function(){return{keywords:null,loading:!1,noresults:!1}},mounted:function(){this.fetch()},computed:{artists:function(){return store.state.artists},results:function(){var _this=this;return this.keywords?store.state.artists.filter(function(result){return result.label.toLowerCase().includes(_this.keywords.toLowerCase())}):[]},hasContent:function(){return this.keywords&&this.keywords.length>0}},methods:{fetch:function(){var obj={api:this.$el.attributes.api.value,types:this.$el.attributes.type.value};if(sessionStorage.getItem(obj.types)){var currentDate=new Date,sessionObject=JSON.parse(sessionStorage.getItem(obj.types)),expirationDate=sessionObject.expiresAt;Date.parse(currentDate)<Date.parse(expirationDate)?store.dispatch("session",obj):store.dispatch("fetchApi",obj)}else store.dispatch("fetchApi",obj)},fetchTouring:function(){var _this2=this;return 0===this.keywords.length?void(this.noresults=!1):void fetch(tourApi+this.keywords).then(function(response){return response.json().then(function(json){return 0===json.length?void(_this2.noresults=!0):(_this2.noresults=!1,void store.dispatch("touringApi",json))})})},highlight:function(text){return text.replace(new RegExp(this.keywords,"gi"),'<span class="highlighted">$&</span>')},tolower:function(text){return text.replace(new RegExp(" ","gi"),"-").toLowerCase()},totouring:function(text){return"/entertainmenttalent/touring/artist/"+text.replace(new RegExp(" ","gi"),"").toLowerCase()},reset:function(){this.noresults=!1,this.keywords=null}}});
